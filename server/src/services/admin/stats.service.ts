...
import { ordersTable, productsTable, productCategoriesTable } from '../../db/schema.ts'
...
export async function getDashboardStats() {
  const [
    ...
    db.select({ count: count() }).from(ordersTable).where(and(eq(ordersTable.status, 'confirmed'), isNull(ordersTable.deletedAt))),
    db.select({ count: count() }).from(ordersTable).where(and(eq(ordersTable.status, 'shipped'), isNull(ordersTable.deletedAt))),
    db.select({ count: count() }).from(ordersTable).where(and(eq(ordersTable.status, 'delivered'), isNull(ordersTable.deletedAt))),
    db.select({ count: count() }).from(ordersTable).where(and(eq(ordersTable.status, 'cancelled'), isNull(ordersTable.deletedAt))),
